...
export default function ClientDrawer({ 
  ...
  interestConfig = {}, 
  fmt,
  currentLang = "fr" // Defaults to French configuration layout template
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      {/* Drawer content surface (Positions dynamically left-0 / right-0 and updates translation rules) */}
      <div 
        dir={t.dir}
        className={`w-[480px] h-full flex flex-col bg-white shadow-2xl overflow-y-auto z-40 fixed top-0 bottom-0 ${t.drawerPosition} ${t.slideAnimate} duration-200`}
      >
        ...
        <div className="flex items-start justify-between p-6 border-b border-[#e2ddd6]">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className={`text-xl text-[#0f1f3d] ${isRTL ? "font-sans font-bold" : "font-serif"}`}>{client.name}</h2>
              <p className="text-[11px] text-gray-400 mt-0.5">
                {client.id} · {t.joinedLabel} {fmt ? fmt(client.joinedDate) : client.joinedDate}
              </p>
            </div>
          </div>
          <button 
            onClick={onClose} 
            className="text-gray-400 hover:text-[#0f1f3d] transition-colors mt-1"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex gap-2 px-6 pt-4 flex-wrap">
          <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[9px] font-bold uppercase tracking-widest ${cfg.bg} ${cfg.color}`}>
            ...
            {t.statuses[client.status] || cfg.label}
          </span>
          <span className={`rounded-full px-3 py-1 text-[9px] font-bold uppercase tracking-widest ${int.color}`}>
            {t.interests[client.interest] || int.label}
          </span>
          {client.tags?.map(tag => (
            <span 
              key={tag} 
              className="rounded-full bg-[#f0ede8] px-3 py-1 text-[9px] font-medium text-[#b89a5a]"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="px-6 pt-5 space-y-3">
          ...
          {[
            { icon: Mail, value: client.email, isLink: false },
            ...
            <div key={idx} className="flex items-center gap-3">
              ...
              <span 
                className="text-[12px] text-[#0f1f3d]" 
                dir={isLink && isRTL ? "ltr" : t.dir}
              >
                {value}
              </span>
            </div>
        ...
        <div className="mx-6 mt-5 grid grid-cols-3 gap-3">
          {[
            ...
            <div key={label} className="rounded-sm bg-[#f7f6f3] border border-[#e2ddd6] p-3 text-center">
              ...
              <p className="text-[11px] font-bold text-[#0f1f3d]" dir={isRaw && isRTL ? "ltr" : t.dir}>{val}</p>
        ...
        {client.dealValue && (
          <div className={`mx-6 mt-4 rounded-sm bg-[#0f1f3d] p-4 border-y-0 border-r-0 border-[#b89a5a] ${isRTL ? "border-r-4 border-l-0" : "border-l-4"}`}>
        ...
        <div className="border-t border-[#e2ddd6] p-4 flex gap-2 mt-auto bg-[#faf9f7]">
